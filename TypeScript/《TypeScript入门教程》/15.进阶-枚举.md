...
## 15.1 枚举上手
枚举使用关键字`enum`来定义
> 过时提醒：`enum` 仍可用，但现代前端项目中经常用字符串字面量联合类型或 `as const` 对象替代，减少运行时代码和打包上的额外负担。
```typescript
...
console.log(Days["Sat"] === 6); // true
```
没有手动赋值的枚举项，会根据前面的枚举项的值响应递增，即使赋值为小数，也递增`1`。
​
...
注意：如果计算所得项后续是未手动赋值的枚举项，那么会因为无法获得初始值而报错。
```typescript
...
- **不具有初始化函数并且之前的枚举成员为常数**，在这种情况下，当前的枚举成员为上一个枚举成员`+1`，但第一个枚举成员是个例外，它如果没有初始函数，那么它的初始值为`0`。
...
```
常数枚举与普通枚举的区别在于：它会在编译阶段被删除，它不可以包含计算所得项。
> 过时提醒：`const enum` 在跨包发布、`isolatedModules`、Babel/SWC 单文件转译等场景中容易踩坑。应用内部可谨慎使用，公共库更建议避免直接暴露 `const enum`。
上面编译成js后：
```javascript
