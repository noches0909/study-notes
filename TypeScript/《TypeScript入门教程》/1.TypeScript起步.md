...
- **弱类型**：运行时允许较多隐式类型转换。TypeScript完全兼容JavaScript，不会修改它的特性。
<!-- 修订注：JavaScript 是动态类型语言，核心原因是类型检查主要发生在运行时；“解释型语言，没有编译过程”不是判断动态类型的依据。现代 JavaScript 引擎通常也会有 JIT 编译等编译阶段。 -->
> 动态类型是指在运行时进行类型检查，JavaScript就是动态类型。
...
> 在用VS Code编写JavaScript时，代码补全、接口提示等功能是通过 TypeScript Language Service实现的。

​

TypeScript与ECMAScript标准同步发展，最新标准的语法它也同样可用。
​
...
> 过时提醒：早期 `tsc` 默认即使有类型错误也可能继续输出编译结果。现在项目里通常会在 `tsconfig.json` 中配置 `"noEmitOnError": true`，或在构建流程中让类型检查失败直接中断发布。
TypeScript编译的时候如果报错了，是否生成编译结果文件取决于 `noEmitOnError` 等配置。
​
