...
- 打包体积小、内存占用少、渲染快
- `Proxy`代替了`defineProperty`实现响应式
...
`setup`钩子函数执行在生命周期`beforeCreate`之前，且每个组件实例创建时执行一次，`setup`中没有组件实例的访问权，即无法使用`this`，该钩子函数有返回值：

- 返回的对象：模板和其他选项中可以直接使用对象的属性和方法
- 返回的函数：函数中可以直接返回渲染函数
...
`ref`创建的响应式数据，在模板中会自动解包，可以直接使用，而在`script`代码中，需要通过`.value`来获取和修改。
...
`computed`计算属性方法返回的值也是一个`ref`，`script`中需要通过`.value`来获取；默认是只读的，只有传入`get`和`set`时才可修改。
...
创建阶段：`beforeCreate`和`created`两个选项式 API 钩子仍然存在，但组合式 API 中通常直接在`setup`中调用需要在此阶段执行的函数。
...
> 过时标记：Vue 3.4 起，组件 `v-model` 推荐使用 `defineModel()` 宏；上面这种 `modelValue` + `update:modelValue` 写法仍可用，适合作为底层原理理解。

关于`$event`：

- 对于原生事件：`$event`即是当前触发事件的事件对象
- 对于自定义事件：`$event`即是当前触发事件所传递的数据

绑定时使用`v-model:test`自定义参数的话，子组件中的`modelValue`要改为自定义参数名`test`，事件名也要改为`update:test`。这样可以在一个组件上绑定多个`v-model`以扩展功能。
...
// 定义一个可传递给后代组件的provide，注意这里的name不能取value，否则会失去响应式
provide("nameContext", { name, updateName })

// 子组件注入：inject可得到祖先组件定义的变量和方法
const { name, updateName } = inject("nameContext", {
  name: ref("默认"),
...
// 页面中引入的 store，是一个响应式对象，取值时自动解包无需`.value`。
const testStore = useTestStore()
console.log(testStore.name)

// 取用多个值时，最好不要使用toRefs，会使无关的store属性和方法也被转化为响应式
// 使用pinia提供的storeToRefs方法代替，storeToRefs只会关注state和getters
...
`readonly`可以传入对象或响应式数据，得到一个只读代理；只读代理无法被直接修改，但如果源数据本身被修改，只读代理也会反映变化。

`shallowReadonly`只会使**浅层**只读，只保证第一层为只读，新数据的深层次可以被修改。

### 9.3 `toRaw`和`markRaw`

`toRaw`能传入一个响应式对象，获得它的原始对象（代理对象变为普通对象）。该原始对象不具备响应式，修改它不会触发视图更新。
...
> 过时/状态标记：截至 Vue 当前官方文档，`<Suspense>` 仍处于实验特性状态，生产中使用要谨慎关注版本变化。
