...
在多核系统创建多个Nodejs进程，充分利用系统的所有cpu核心，提高性能。

单线程限制：Nodejs是基于单线程的事件驱动模型，默认只能使用一个cpu核心

多进程架构：通过集群模块，可以创建多个Nodejs进程，每个进程处理部分能量，从而利用多个cpu核心

## 部署pm2

Nodejs进程管理工具

进程管理、负载均衡、自动重启、日志管理、监控和性能分析、配置文件支持、热重载、容器支持
